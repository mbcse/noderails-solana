...
export function OnboardingWizard({ onComplete }: { onComplete: (appId: string) => void }) {
  ...
  // Step 3 — EVM (wagmi) + Solana (wallet-adapter), both optional but encouraged
  const [evmWalletData, setEvmWalletData] = useState<{ address: string; signature: string } | null>(
    null,
  );
  const [solWalletData, setSolWalletData] = useState<{ address: string; signatureBase64: string } | null>(
    null,
  );
  ...
  async function handleWalletComplete() {
    if (!token || !createdApp) return;
    if (!evmWalletData && !solWalletData) return;
    setSaving(true);
    setError('');
    try {
      if (evmWalletData) {
        await api.updateApp(token, createdApp.id, {
          receivingWallet: evmWalletData.address,
          receivingWalletSignature: evmWalletData.signature,
        });
      } else if (solWalletData) {
        await api.updateApp(token, createdApp.id, {
          receivingWallet: solWalletData.address,
          receivingWalletSignature: solWalletData.signatureBase64,
        });
      }

      if (solWalletData) {
        const chains = await api.getAvailableChains(token, appEnv);
        const solChain = chains.find((c: { chainType?: string }) => c.chainType === 'SOLANA');
        if (solChain) {
          const chainId = String(solChain.chainId);
          await api.enableAppChain(token, createdApp.id, chainId);
          await api.updateAppChainSettlement(
            token,
            createdApp.id,
            chainId,
            solWalletData.address,
          );
        }
      }
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  <strong>Receiving wallet EVM</strong> and <strong>Receiving wallet Solana</strong>: connect, then sign to verify. Finish after at least one, or skip.
                </p>
              </div>

              <div className="space-y-4 rounded-xl border border-border bg-muted/30 p-4">
                <div className="space-y-2">
                  <p className="text-xs font-semibold text-foreground">Receiving wallet EVM</p>
                  <WalletConnect
                    appName={createdApp?.name ?? 'App'}
                    onWalletVerified={(address, signature) => setEvmWalletData({ address, signature })}
                  />
                </div>
                <div className="border-t border-border pt-4 space-y-2">
                  <p className="text-xs font-semibold text-foreground">Receiving wallet Solana</p>
                  <SolanaWalletConnect
                    appName={createdApp?.name ?? 'App'}
                    appEnv={appEnv}
                    onVerified={(address, signatureBase64) =>
                      setSolWalletData({ address, signatureBase64 })
                    }
                  />
                </div>
              </div>
