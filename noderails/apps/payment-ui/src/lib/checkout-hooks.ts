...
export function useTokenBalance(
  ...
  const { data: erc20Balance, refetch: refetchErc20 } = useReadContract({
    ...
    chainId,
    query: {
      enabled: !!address && !!tokenAddress && !!chainId && !isNative,
...
export function useTokenAllowance(
  ...
  const { data: allowance, refetch } = useReadContract({
    ...
    chainId,
    query: {
      enabled: !!address && !!tokenAddress && !!spenderAddress && !!chainId && !isNative,
      refetchInterval: 5_000,
    },
  });

  return { allowance: isNative ? undefined : allowance, refetch };
}
