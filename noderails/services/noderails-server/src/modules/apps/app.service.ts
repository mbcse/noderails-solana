...
import { NotFoundError, AuthorizationError, ValidationError, isValidAddress, isValidSolanaAddress, isValidMerchantWalletAddress } from '@noderails/common';
...
export async function updateApp(merchantId: string, appId: string, input: UpdateAppInput) {
  ...
  if (
    ...
    throw new ValidationError('Invalid receiving wallet (use 0x… for EVM or a Solana address)');
  }
  if (
    ...
    throw new ValidationError('Invalid payout wallet (use 0x… for EVM or a Solana address)');
  }
