...
export async function acceptInvite(inviteToken: string, password: string) {
  ...
  return { tokens, member: { id: member.id, email: member.email, permissions: member.permissions, allAppsAccess: member.allAppsAccess, appIds: member.appAccess.map(a => a.appId), merchantId: member.merchantId } };
}
