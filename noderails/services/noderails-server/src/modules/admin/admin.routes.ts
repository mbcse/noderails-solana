...
import { AuthenticationError, AUTH_CONFIG, AddressSchema, SolanaAddressSchema } from '@noderails/common';
...
const createChainSchema = z
  .object({
    ...
    escrowAddress: z.string().min(1).max(64),
    merchantManagerAddress: z.string().min(1).max(64),
  ...
  .superRefine((data, ctx) => {
    const check = (path: 'escrowAddress' | 'merchantManagerAddress', val: string) => {
      const schema = data.chainType === 'SOLANA' ? SolanaAddressSchema : AddressSchema;
        ...
        ctx.addIssue({
          ...
          message:
            data.chainType === 'SOLANA'
              ? 'Invalid Solana program id (base58)'
              : 'Invalid EVM contract address',
          path: [path],
        });
      }
    };
    check('escrowAddress', data.escrowAddress);
    check('merchantManagerAddress', data.merchantManagerAddress);
  });

const updateChainSchema = z.object({
  chainType: z.enum(['EVM', 'SOLANA']).optional(),
  name: z.string().min(1).max(100).optional(),
  displayName: z.string().min(1).max(100).optional(),
  nativeCurrencySymbol: z.string().min(1).max(10).optional(),
  nativeCurrencyDecimals: z.number().int().min(0).max(18).optional(),
  escrowAddress: z.string().min(1).max(64).optional(),
  merchantManagerAddress: z.string().min(1).max(64).optional(),
...
const createTokenSchema = z.object({
  chainId: z.number().int().positive(),
  contractAddress: z.string().min(1).max(64),
