...
function normalizeAdminTokenContractAddress(chainType: ChainType, contractAddress: string): string {
  ...
  if (chainType !== ChainType.SOLANA) return trimmed;
...
export async function createChain(input: CreateChainInput) {
  ...
  const chainType = input.chainType === 'SOLANA' ? ChainType.SOLANA : ChainType.EVM;
  ...
  return db.supportedChain.create({
    data: {
      ...
      nativeCurrencyDecimals: input.nativeCurrencyDecimals ?? 18,
      escrowAddress: input.escrowAddress,
      merchantManagerAddress: input.merchantManagerAddress,
      mtxmChainDbId: input.mtxmChainDbId ?? undefined,
      rpcUrl:
        input.rpcUrl ??
        (chainType === ChainType.SOLANA ? null : getLeanRpcUrl(input.chainId)),
      explorerUrl: input.explorerUrl,
      isTestnet: input.isTestnet ?? false,
      supports7702: chainType === ChainType.SOLANA ? false : (input.supports7702 ?? false),
